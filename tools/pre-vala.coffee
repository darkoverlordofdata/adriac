...
template = (file, name, options) ->
    ...
    ##
    if ///^(\s*)public\s+class\s+#{klass}\s*\<\w,\s*\w\>\s*:\s*Object\s{*///m.test src
        src = src.replace(///^(\s*)public\s+class\s+(#{klass})\s*\<(\w,\s*\w)\>\s*:\s*(Object)\s*{///m, ($0, $1, $2, $3, $4) ->
            tab = $1.replace(/\n/mg, "").replace("\t", "")
            """
...
#{tab}\tpublic extern void free();\n\t\t
    ...
    ##  class Name <G> : Object {
    ##
    else if ///^(\s*)public\s+class\s+#{klass}\s*\<\w\>\s*:\s*Object\s{*///m.test src
        src = src.replace(///^(\s*)public\s+class\s+(#{klass})\s*\<(\w)\>\s*:\s*(Object)\s*{///m, ($0, $1, $2, $3, $4) ->
            tab = $1.replace(/\n/mg, "").replace("\t", "")
            """
#{tab}[Compact, CCode ( /** reference counting */
#{tab}\tref_function = "#{pfx}_retain", 
...
#{tab}\tpublic extern void free();\n\t\t
    ...
    ##  class Name : Object {
    ##
    else if ///^(\s*)public\s+class\s+#{klass}\s*:\s*Object\s{*///m.test src
        src = src.replace(///^(\s*)public\s+class\s+(#{klass})\s*:\s*(Object)\s*{///m, ($0, $1, $2, $3) ->
            tab = $1.replace(/\n/mg, "").replace("\t", "")
            """
#{tab}[Compact, CCode ( /** reference counting */
#{tab}\tref_function = "#{pfx}_retain", 
...
#{tab}\tpublic extern void free();\n\t\t
    ...
    ##  class Name {
    ##
    else if ///^(\s*)public\s+class\s+#{klass}\s*:\s*\w+\s*{///m.test src
        src = src.replace(///^(\s*)public\s+class\s+(#{klass})\s*:\s*(\w+)\s*{///m, ($0, $1, $2, $3) ->
            tab = $1.replace(/\n/mg, "").replace("\t", "")
            """
#{tab}[Compact]
#{tab}public class #{$2} : #{$3} {\n#{tab}\t
