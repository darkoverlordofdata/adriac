 ...
 * @return string
 */
public string publicAccess(string s) {
...
/**
 * generateMacroWrapper
 * 
 * @param string line
 * @param HashMap<string,string> symtbl
 * @param StringBuilder dst
 */
public bool generateMacroWrapper(string line, HashMap<string,string> symtbl, StringBuilder dst) {
	var flag = false;
	var t = /\#define\s+_([_a-z0-9]+)_release0/.replace_eval(line, line.length, 0, 0, (info, result) => {

		//  print("DEFINE %s / %s\n", info.fetch(0), info.fetch(1));
		var p1 = info.fetch(1);
 ...
 * @param StringBuilder dst
 */
public bool generateFunctionWrapper(string line, HashMap<string,string> symtbl, StringBuilder dst) {
	var flag = false;
	var t = /static\s+void\s+_([_a-z0-9]+)_release0_/.replace_eval(line, line.length, 0, 0, (info, result) => {
 ...
 * @param StringBuilder dst
 */
public bool generateBaseMethods(string line, HashMap<string,string> symtbl, StringBuilder dst) {
	var flag = false;
	var t = /(\w+)\*\s+([_a-z0-9]+)_new/.replace_eval(line, line.length, 0, 0, (info, result) => {
		
		//  print("METHOD %s / %s / %s\n", info.fetch(0), info.fetch(1), info.fetch(2));
		var p1 = info.fetch(1);
		var p2 = info.fetch(2);
		if (symtbl.has_key(p1)) {
			var type = symtbl.@get(p1);
