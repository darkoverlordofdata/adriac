...
#ifndef _GLIB_NODE_H_
#define _GLIB_NODE_H_
...
static GNode*	 g_node_new		(gpointer	   data);

static void	 g_node_destroy		(GNode		  *root);

static void	 g_node_unlink		(GNode		  *node);

static GNode*   g_node_copy_deep       (GNode            *node,
				 GCopyFunc         copy_func,
				 gpointer          data);

static GNode*   g_node_copy            (GNode            *node);

static GNode*	 g_node_insert		(GNode		  *parent,
				 gint		   position,
				 GNode		  *node);

static GNode*	 g_node_insert_before	(GNode		  *parent,
				 GNode		  *sibling,
				 GNode		  *node);
...
static GNode*	 g_node_prepend		(GNode		  *parent,
				 GNode		  *node);

static guint	 g_node_n_nodes		(GNode		  *root,
				 GTraverseFlags	   flags);

static GNode*	 g_node_get_root	(GNode		  *node);

static gboolean g_node_is_ancestor	(GNode		  *node,
				 GNode		  *descendant);

static guint	 g_node_depth		(GNode		  *node);

static GNode*	 g_node_find		(GNode		  *root,
				 GTraverseType	   order,
				 GTraverseFlags	   flags,
				 gpointer	   data);
...
static void	 g_node_traverse	(GNode		  *root,
				 GTraverseType	   order,
				 GTraverseFlags	   flags,
				 gint		   max_depth,
...
static guint	 g_node_max_height	 (GNode *root);


static void	 g_node_children_foreach (GNode		  *node,
				  GTraverseFlags   flags,
				  GNodeForeachFunc func,
				  gpointer	   data);

static void	 g_node_reverse_children (GNode		  *node);

static guint	 g_node_n_children	 (GNode		  *node);

static GNode*	 g_node_nth_child	 (GNode		  *node,
				  guint		   n);

static GNode*	 g_node_last_child	 (GNode		  *node);

static GNode*	 g_node_find_child	 (GNode		  *node,
				  GTraverseFlags   flags,
				  gpointer	   data);

static gint	 g_node_child_position	 (GNode		  *node,
				  GNode		  *child);

static gint	 g_node_child_index	 (GNode		  *node,
				  gpointer	   data);


static GNode*	 g_node_first_sibling	 (GNode		  *node);

static GNode*	 g_node_last_sibling	 (GNode		  *node);
