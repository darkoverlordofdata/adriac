...
#ifndef _GLIB_ARRAY_H__
#define _GLIB_ARRAY_H__
...
static GArray* g_array_new               (gboolean          zero_terminated,
				   gboolean          clear_,
				   guint             element_size);

static GArray* g_array_sized_new         (gboolean          zero_terminated,
				   gboolean          clear_,
				   guint             element_size,
				   guint             reserved_size);

static gchar*  g_array_free              (GArray           *array,
				   gboolean          free_segment);

static GArray *g_array_ref               (GArray           *array);

static void    g_array_unref             (GArray           *array);

static guint   g_array_get_element_size  (GArray           *array);

static GArray* g_array_append_vals       (GArray           *array,
				   gconstpointer     data,
				   guint             len);

static GArray* g_array_prepend_vals      (GArray           *array,
				   gconstpointer     data,
				   guint             len);

static GArray* g_array_insert_vals       (GArray           *array,
				   guint             index_,
				   gconstpointer     data,
				   guint             len);

static GArray* g_array_set_size          (GArray           *array,
				   guint             length);

static GArray* g_array_remove_index      (GArray           *array,
				   guint             index_);

static GArray* g_array_remove_index_fast (GArray           *array,
				   guint             index_);

static GArray* g_array_remove_range      (GArray           *array,
				   guint             index_,
				   guint             length);

// static void    g_array_sort              (GArray           *array,
// 				   GCompareFunc      compare_func);

// static void    g_array_sort_with_data    (GArray           *array,
// 				   GCompareDataFunc  compare_func,
// 				   gpointer          user_data);

static void    g_array_set_clear_func    (GArray           *array,
...
static GPtrArray* g_ptr_array_new_with_free_func (GDestroyNotify    element_free_func);

static GPtrArray* g_ptr_array_sized_new          (guint             reserved_size);

static GPtrArray* g_ptr_array_new_full           (guint             reserved_size,
					   GDestroyNotify    element_free_func);

static gpointer*  g_ptr_array_free               (GPtrArray        *array,
					   gboolean          free_seg);

static GPtrArray* g_ptr_array_ref                (GPtrArray        *array);

static void       g_ptr_array_unref              (GPtrArray        *array);

static void       g_ptr_array_set_free_func      (GPtrArray        *array,
                                           GDestroyNotify    element_free_func);

static void       g_ptr_array_set_size           (GPtrArray        *array,
					   gint              length);

static gpointer   g_ptr_array_remove_index       (GPtrArray        *array,
					   guint             index_);

static gpointer   g_ptr_array_remove_index_fast  (GPtrArray        *array,
					   guint             index_);

static gboolean   g_ptr_array_remove             (GPtrArray        *array,
					   gpointer          data);

static gboolean   g_ptr_array_remove_fast        (GPtrArray        *array,
					   gpointer          data);

static GPtrArray *g_ptr_array_remove_range       (GPtrArray        *array,
					   guint             index_,
					   guint             length);

static void       g_ptr_array_add                (GPtrArray        *array,
					   gpointer          data);
// GLIB_AVAILABLE_IN_2_40
static void       g_ptr_array_insert             (GPtrArray        *array,
                                           gint              index_,
                                           gpointer          data);

// static void       g_ptr_array_sort               (GPtrArray        *array,
// 					   GCompareFunc      compare_func);

// static void       g_ptr_array_sort_with_data     (GPtrArray        *array,
// 					   GCompareDataFunc  compare_func,
// 					   gpointer          user_data);

static void       g_ptr_array_foreach            (GPtrArray        *array,
					   GFunc             func,
					   gpointer          user_data);
...
static GByteArray* g_byte_array_new_take          (guint8           *data,
                                            gsize             len);

static GByteArray* g_byte_array_sized_new         (guint             reserved_size);

static guint8*     g_byte_array_free              (GByteArray       *array,
					    gboolean          free_segment);

// static GBytes*     g_byte_array_free_to_bytes     (GByteArray       *array);

static GByteArray *g_byte_array_ref               (GByteArray       *array);

static void        g_byte_array_unref             (GByteArray       *array);

static GByteArray* g_byte_array_append            (GByteArray       *array,
					    const guint8     *data,
					    guint             len);

static GByteArray* g_byte_array_prepend           (GByteArray       *array,
					    const guint8     *data,
					    guint             len);

static GByteArray* g_byte_array_set_size          (GByteArray       *array,
					    guint             length);

static GByteArray* g_byte_array_remove_index      (GByteArray       *array,
					    guint             index_);

static GByteArray* g_byte_array_remove_index_fast (GByteArray       *array,
					    guint             index_);

static GByteArray* g_byte_array_remove_range      (GByteArray       *array,
					    guint             index_,
					    guint             length);

// static void        g_byte_array_sort              (GByteArray       *array,
// 					    GCompareFunc      compare_func);

// static void        g_byte_array_sort_with_data    (GByteArray       *array,
// 					    GCompareDataFunc  compare_func,
// 					    gpointer          user_data);
            ...
            ArrayFreeFlags  flags)
{
  ...
  if (flags & PRESERVE_WRAPPER)
    ...
    {
      g_slice_free (sizeof (GRealArray), array);
    }
                ...
                ArrayFreeFlags  flags)
{
  ...
  if (flags & PRESERVE_WRAPPER)
    ...
    {
      g_slice_free (sizeof (GRealPtrArray), rarray);
    }
